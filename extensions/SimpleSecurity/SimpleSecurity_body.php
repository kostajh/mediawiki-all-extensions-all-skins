...
class SimpleSecurity {
	...
	public static function onParserFirstCallInit( $parser ) {
		$parser->setFunctionHook( 'ifusercan',    array( __CLASS__, 'ifUserCan' ) );
	 ...
	 */
	public static function ifUserCan( &$parser, $action, $pagename, $then, $else = '' ) {
		$title = Title::newFromText( $pagename );
		if ( class_exists( 'MediaWiki\Permissions\PermissionManager' ) ) {
			// MW 1.33+
			$userCan = \MediaWiki\MediaWikiServices::getInstance()
				->getPermissionManager()
				->userCan( $action, $parser->getUser(), $title );
		} else {
			$userCan = $title->userCan( $action );
		}
		return $userCan ? $then : $else;
	}
	 ...
	 */
	public static function ifGroup( &$parser, $groups, $then, $else = '' ) {
		$intersection = array_intersect( array_map( 'strtolower', explode( ',', $groups ) ), $parser->getUser()->getEffectiveGroups() );
	...
	public function onOutputPageBeforeHTML( &$out, &$text ) {
		...
		if ( is_object( $title ) && $title->exists() && count( $this->info['LS'] ) + count( $this->info['PR'] ) ) {
			...
			$sysop = in_array( 'sysop', $user->getGroups() );
	...
	public function onImgAuthBeforeStream( &$title, &$path, &$name, &$result ) {
		...
		if ( !$this->userCanReadTitle( $user, $title, $error )) {
