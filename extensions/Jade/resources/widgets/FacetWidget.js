...
var FacetWidget = function ( config ) {
	...
	this.label1 = new OO.ui.LabelWidget( {
		...
		label: new OO.ui.HtmlSnippet( $( '<b>' ).text( this.getFacetName( this.facetName ) )
			.add( $( '<span>' )
				.addClass( 'jade-facetWidget-header-editTalkLinks' )
				.html( mw.message( 'jade-facetwidget-header-edittalklinks', mw.config.values.entityTitle.mTextform ).parse() ) ) ),
		align: 'left'
	} );
	...
	this.facetHeader.addItems( [
		...
		new OO.ui.FieldLayout( this.label1, {
			help: new OO.ui.HtmlSnippet( mw.message( 'jade-facet-editquality-desc' ).parse() ),
