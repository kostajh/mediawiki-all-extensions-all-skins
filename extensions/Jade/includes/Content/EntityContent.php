...
class EntityContent extends JsonContent {
	...
	public function fillParserOutput(
		...
		if ( $generateHtml ) {
			...
			$entityBuilder = new EntityBuilder( User::newFromName( 'jade_extension' ) );
			$entityDataWithParsedWikitext =
				$entityBuilder->parseWikitextInProposalNotesAndEndorsementComments(
					$entityData,
					$title
				);
			global $wgServer;
