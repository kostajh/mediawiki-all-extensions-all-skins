...
class EntityBuilder {
	 ...
	 * @return string|null
	 */
	public function parseWikitext( $text, $title ) {
	...
	/**
	 * Parse Wikitext in entityData proposal notes and endorsement comments
	 *
	 * @param array $data
	 * @param Title $title
	 * @return array|null
	 */
	public function parseWikitextInProposalNotesAndEndorsementComments( $data, $title ) {
		$data = json_decode( json_encode( $data ), true );
		if ( !empty( $data[ 'facets' ] [ 'editquality' ] [ 'proposals' ] ) ) {
			foreach (
				$data[ 'facets' ] [ 'editquality' ] [ 'proposals' ] as $proposalKey => $proposalValue
			) {
				$proposal = $data[ 'facets' ] [ 'editquality' ] [ 'proposals' ][ $proposalKey ];
				if ( !empty( $proposal[ 'notes' ] ) && empty( $proposal[ 'notesWithParsedWikitext' ] ) ) {
					$notesWithParsedWikitext = $this->parseWikitext( $proposal[ 'notes' ], $title );
					$data[ 'facets' ] [ 'editquality' ] [ 'proposals' ][ $proposalKey ][ 'notesWithParsedWikitext' ] =
						$notesWithParsedWikitext;
				}
				if ( !empty( $proposal[ 'endorsements' ] ) ) {
					foreach ( $proposal[ 'endorsements' ] as $endorsementKey => $endorsementValue ) {
						if ( empty( $data[ 'facets' ] [ 'editquality' ] [ 'proposals' ][ $proposalKey ]
							[ 'endorsements' ][ $endorsementKey ][ 'commentWithParsedWikitext' ] ) ) {
							$commentsWithParsedWikitext =
								$this->parseWikitext(
									$proposal[ 'endorsements' ][ $endorsementKey ][ 'comment' ],
									$title
								);
							$data[ 'facets' ] [ 'editquality' ] [ 'proposals' ][ $proposalKey ]
								[ 'endorsements' ][ $endorsementKey ][ 'commentWithParsedWikitext' ] =
									$commentsWithParsedWikitext;
						}
					}
	...
	/**
	 * Load a Jade Entity on a secondary integration page.
	 *
	 * @param int $diffID The latest id in this revision.
	 * @param string $secondaryIntegrationPageName e.g 'specialDiffPage', 'undoEditPage'
	 * @param OutputPage $output The OutputPage object ($wgOut).
	 */
	public function loadEntityOnSecondaryIntegrationPage(
		$diffID,
		$secondaryIntegrationPageName,
		OutputPage $output
	) {
		$entityTitleStr = 'Diff/' . $diffID;
		$entity = $this->loadEntityPage( $entityTitleStr );
		if ( !empty( $entity ) ) {
			$entityData = $entity[1];
			$entityPage = $entity[0];
			$entityTitle = $entityPage->getTitle();
		} else {
			$entityData = json_encode( json_decode( '{}' ) );
			$entityTitleLink = new TitleValue( NS_JADE, $entityTitleStr );
			$entityTitle = Title::newFromLinkTarget( $entityTitleLink );
			$entityTitle->mArticleID = 0;
			$entityTitle->prefixedText = 'Jade:' . $entityTitleStr;
		}
		$entityDataWithParsedWikitext =
			$this->parseWikitextInProposalNotesAndEndorsementComments( $entityData, $entityTitle );
		$entityConfigVars = [
			'entityData' => $entityDataWithParsedWikitext,
			'entityTitle' => [
				'prefixedText' => $entityTitle->getPrefixedText(),
				'mTextForm' => $entityTitle->getText()
			],
			'jadeSecondaryIntegrationPage' => $secondaryIntegrationPageName
		];
		$output->enableClientCache( false );
		$output->addJsConfigVars( $entityConfigVars );
		$output->addModules( [
			'ext.Jade.entityView',
			'jade.api',
			'jade.widgets',
