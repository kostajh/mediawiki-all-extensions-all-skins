...
class EntityLinkTableTest extends MediaWikiTestCase {
	...
	protected function setUp() : void {
		...
		$this->revisionRecord = $article['revision-record'];
	...
	private function createJudgment( $entityType, $entityId ) {
		...
		$this->assertNotNull( $judgmentStatus['revision-record'] );
	...
	public function testFixtures() {
		$judgment = $this->createJudgment( 'diff', $this->revisionRecord->getId() );
	...
	public function testInsertIndex_normal( $type ) {
		$judgment = $this->createJudgment( $type, $this->revisionRecord->getId() );
	...
	public function testInsertIndex_duplicate( $type ) {
		...
		$result = TestLinkTableHelper::selectJudgmentLink(
			$type, $this->revisionRecord->getId(), $judgment['judgmentPage']->getId() );
	...
	public function testUpdateSummary_normal() {
		...
		// Prepare the row.
		$this->indexStorage->insertIndex( $judgment['target'], $judgment['judgmentPage'] );
		$this->assertJudgmentLink( 'diff', $this->revisionRecord->getId(), $judgment['judgmentPage']->getId() );
		...
		foreach ( $result as $row ) {
			$this->assertSame( '1', $row->jadedl_damaging );
