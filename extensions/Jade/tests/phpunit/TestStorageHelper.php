...
class TestStorageHelper {
	 ...
	 * @param User $user User to save as.
	 *
	 * @return StatusValue
	 */
	public static function saveJudgment( $titleStr, $text, $user ) {
		if ( is_array( $text ) ) {
	 ...
	 *        or a failure.
	 *
	 * @return array [ 'page' => WikiPage, 'revision-record' => RevisionRecord ]
	 */
	public static function makeEdit(
		...
		$revisionRecord = $status->value['revision-record'];
		Assert::assertNotNull( $revisionRecord, 'No revision after edit' );
