...
class SmoothGallery {
	...
	function checkForErrors() {
		foreach ( $this->galleriesArray["galleries"] as $galleryArray ) {
			...
			if ( !isset( $galleryArray["images"] ) ) {
				...
				$error = wfMessage( "smoothgallery-error" )->text();

				if ( isset( $galleryArray["missing_images"] )  && isset( $galleryArray["invalid_images"] ) ) {
					$error .= wfMessage( "smoothgallery-no-images", implode( ", ", $galleryArray["missing_images"] ), count( $galleryArray["missing_images"] ) + count( $galleryArray["invalid_images"] ) )->escaped();	// FIXME: 3rd (last) parameter should have the number of good images added.
					$error .= wfMessage( "smoothgallery-invalid-images", implode( ", ", $galleryArray["invalid_images"] ) )->escaped();
				} elseif ( isset( $galleryArray["invalid_images"] ) ) {
					$error .= wfMessage( "smoothgallery-invalid-images", implode( ", ", $galleryArray["invalid_images"] ) )->escaped();
				} elseif ( isset( $galleryArray["missing_images"] ) ) {
					$error .= wfMessage( "smoothgallery-no-images", implode( ", ", $galleryArray["missing_images"] ), count( $galleryArray["missing_images"] ) )->escaped();	// FIXME: 3rd (last) parameter should have the number of good images added.
				} else {
					$error .= wfMessage( "smoothgallery-not-found" )->escaped();
				}
	...
	function renderFallback ( $galleryArray ) {
		...
		if ( $this->argumentArray["fallback"] == "image" ) {
			...
			$output .= wfMessage( "smoothgallery-javascript-disabled" )->escaped();
	...
	function renderJavascript ( $name ) {
		...
		# $output .= ", textPreloadingCarousel: '" . wfMessage("smoothgallery-loading")->escaped() . "'";
